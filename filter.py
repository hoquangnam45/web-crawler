...
# Filter by voting
def applyDecisions(contents: list[str], *decisionsList: list[bool], method=VotingDecision.YES_ONE) -> list[str]:
    ret: list[str] = []
...
def reduceDecisions(decisions: list[bool], method=VotingDecision.YES_ONE) -> bool:
    if method == VotingDecision.YES_ONE:
        for decision in decisions:
            if decision:
                return True
    
    if method == VotingDecision.ALL:
        for decision in decisions:
            if decision is False:
                return False
        return True
    
    if method == VotingDecision.MAJORITY:
